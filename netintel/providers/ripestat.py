...
async def as_overview(*, client: httpx.AsyncClient, asn: int) -> Dict[str, Any]:
    return await _get(client, "/as-overview/data.json", {"resource": f"AS{asn}", "sourceapp": "netintel"})


async def abuse_contact(*, client: httpx.AsyncClient, asn: int) -> Dict[str, Any]:
    return await _get(client, "/abuse-contact-finder/data.json", {"resource": f"AS{asn}", "sourceapp": "netintel"})


async def routing_status(*, client: httpx.AsyncClient, asn: int) -> Dict[str, Any]:
    return await _get(client, "/routing-status/data.json", {"resource": f"AS{asn}", "sourceapp": "netintel"})


async def asn_neighbours(*, client: httpx.AsyncClient, asn: int) -> Dict[str, Any]:
    return await _get(client, "/asn-neighbours/data.json", {"resource": f"AS{asn}", "sourceapp": "netintel"})


async def announced_prefixes(*, client: httpx.AsyncClient, asn: int) -> Dict[str, Any]:
    return await _get(client, "/announced-prefixes/data.json", {"resource": f"AS{asn}", "sourceapp": "netintel"})
